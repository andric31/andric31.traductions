...
export async function onRequest(context) {
  try {
    ...
    if (!expectedToken) {
      return json({ ok: false, error: 'AUTH_ADMIN_TOKEN non configuré.' }, 500);
    }
    ...
    if (!username || username.length < 3) {
      return json({ ok: false, error: 'Nom d’utilisateur invalide.' }, 400);
    }

    const pwError = validatePassword(password);
    if (pwError) {
      return json({ ok: false, error: pwError }, 400);
    }

    const exists = await env.DB
      .prepare(`SELECT id FROM auth_users WHERE username = ?1 LIMIT 1`)
      .bind(username)
      .first();

    if (exists) {
      return json({ ok: false, error: 'Cet utilisateur existe déjà.' }, 409);
    }
    ...
    try {
      ...
      return json({
        ok: false,
        error: 'Erreur pendant le hash du mot de passe.',
    ...
    return new Response(JSON.stringify({
      ok: false,
      error: 'Erreur interne auth-admin-create-user',
      detail: String(e?.message || e || 'unknown'),
    }), {
      status: 500,
      headers: {
