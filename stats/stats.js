...
function getFiltered(){
  ...
  if (q){
    list = list.filter(g => {
      const hay = normalize([
        g.id, g.uid, g.title, g.cleanTitle,
        (g.tags||[]).join(" "),
        g.collection || ""
      ].join("  "));
  ...
  for (const g of list){
    ...
    g._views = s.views|0; g._likes = s.likes|0; g._mega = s.mega|0;
  }
  return list;
}
...
function renderTable(list){
  ...
  for (const g of list){
    ...
    catTd.innerHTML = `<span class="badge-mini">${escapeHtml(g._cat || "")}</span>`;

    const engTd = document.createElement("td");
    engTd.innerHTML = `<span class="badge-mini">${escapeHtml(g._engine || "")}</span>`;
...
function drawChart(list){
  ...
  const padL = 260, padR = 18, padT = 16, padB = 28;
  const W = cssW, H = cssH;
...
function rerender(){
  ...
  // top slicing for chart/table ? we keep full table, chart uses top selector
  renderTable(sorted);
  drawChart(sorted);

  const total = state.games.length;
  els.status.textContent = `${sorted.length}/${total} jeux (source: ${shortUrl(state.srcUrl)})`;
}
...
function wireEvents(){
  let t = null;
  const deb = ()=>{ clearTimeout(t); t = setTimeout(rerender, 120); };

  els.q.addEventListener("input", deb);
  els.metric.addEventListener("change", rerender);
  els.cat.addEventListener("change", rerender);
  els.eng.addEventListener("change", rerender);
  els.top.addEventListener("change", rerender);
  ...
  els.tbl.querySelectorAll("thead th[data-sort]").forEach(th=>{
    th.addEventListener("click", ()=>{
      ...
      if (state.sortKey === k){
        ...
        state.sortKey = k;
        state.sortDir = (k === "title" || k === "cat" || k === "engine" || k === "updatedAt") ? "asc" : "desc";
      }
      rerender();
    });
  });
